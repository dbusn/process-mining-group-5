...
f.write("DBL Process Mining Demo tool v4\n")
...
display_top(mem_snapshot)
tracemalloc.stop()


for i in tqdm(range(memory_interval)):
    time.sleep(1)
...
mae_test = sum((abs(df_test['baseline_predicted_time'] - df_test['time_diff']))) / len(df_test['time_diff'])

metrics_out = open("metrics.txt", "a")

metrics_out.write('Baseline algorithm B MSE: ' + str(round(mse_test,2)) + '\n')
metrics_out.write('Baseline algorithm B MAE: ' + str(round(mae_test,2)) + '\n')
metrics_out.close()

print('BASELINE ALGORITHM B MSE: ', str(round(mse_test,2)))
print('BASELINE ALGORITHM B MAE: ', str(round(mae_test,2)))
...
vreg_mae = str(round(metrics.mean_absolute_error(voting_pred, y_test[:n]), 2))
vreg_rmse = str(
    round(metrics.mean_squared_error(voting_pred, y_test[:n], squared=False), 2)
)
...
model = train(train_log, epochs=50, early_stop=5)
...
# Save the results
result_df.to_csv("tool_v4_predictions.csv")
